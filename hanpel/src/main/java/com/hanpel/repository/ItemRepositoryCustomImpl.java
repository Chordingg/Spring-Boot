...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom{ // ItemRepositoryCustom 상속 받는다

    private JPAQueryFactory queryFactory; // 동적으로 쿼리를 생성하기 위해서 JPAQueryFactory 클래스 사용
    ...
    private BooleanExpression searchSellStatusEq(ItemSellStatus searchSellStatus) {
        ...
        // searchDateType 의 값에 따라서...
        return searchSellStatus == null ? null : QItem.item.itemSellStatus.eq(searchSellStatus);
    }

    // searchDateType 의 값에 따라서 dateTime 의 값을 이전 시간의 값으로 세팅 후 해당 시간 이후로 등록된 상품만 조회
    // searchDateType = "1m"인 경우 dateTime 의 시간을 한 달 전으로 세팅 후 한 달 동안 등록된 상품만 조회하도록 조건값 반환
    ...
    // searchBy 의 값에 따라서 상품명에 검색어를 포함하고 있는 상품 또는 상품 생성자의 아이디에 검색어를 포함하고 있는 상품을 조회하도록 조건값을 반환
    ...
    public Page<Item> getAdminItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
        ...
        where : BooleanExpression 반환하는 조건문을 넣어준다. ',' 은 and 조건으로 인식
        offset : 데이터를 가지고 올 시작 인덱스를 지정
        limit : 한 번에 가지고 올 최대 개수를 지정
        fetchResult() : 조회한 리스트 및 전체 개수를 포함하는 QueryResults 를 반환. 상품 데이터 리스트 조회 및 상품 데이터 전체 개수를 조회하는 2번의 쿼리문 실행 
    ...
    public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
        ...
        QueryResults<MainItemDto> results = queryFactory
                // QMainItemDto 의 생성자에 반환할 값들을 넣어준다.
                // @QueryProject 을 사용하면 DTO 로 바로 조회가 가능.
